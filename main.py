import streamlit as st
import hydralit_components as hc
import os
from datetime import datetime, timedelta
import pandas as pd
from difflib import get_close_matches
from streamlit_extras.metric_cards import style_metric_cards
from streamlit_extras.stylable_container import stylable_container 

# --- APP CONFIG ---
st.set_page_config(page_title="CCSS School Portal", page_icon="🏫", layout="wide")

# --- CSS LOADER ---
def local_css(file_name):
    if os.path.exists(file_name):
        with open(file_name) as f:
            st.markdown(f'<style>{f.read()}</style>', unsafe_allow_html=True)

local_css("style.css")

# --- DATA LOADING ---
@st.cache_data
def load_students():
    students_dict = {}
    if os.path.exists("student.txt"):
        with open("student.txt", "r") as f:
            for line in f:
                if "," in line:
                    name, room = line.strip().split(",")
                    students_dict[name.lower().strip()] = room.strip()
    return students_dict

def load_detention_data():
    if os.path.exists("detention.txt") and os.stat("detention.txt").st_size > 0:
        return pd.read_csv("detention.txt", names=["Student", "Room", "Time", "Date"])
    return pd.DataFrame(columns=["Student", "Room", "Time", "Date"])

students = load_students()
history_df = load_detention_data()

# Time Adjustment for Saint Lucia (UTC-4)
school_time = datetime.now() - timedelta(hours=4)
today_str = school_time.strftime("%Y-%m-%d")

# --- RED NAVIGATION BAR ---
menu_data = [
    {'icon': "bi bi-house", 'label': "Home"},
    {'icon': "bi bi-person-check", 'label': "Student Check-in"},
    {'icon': "bi bi-shield-lock", 'label': "Teacher Attendance"},
]

over_theme = {'txc_inactive': '#FFFFFF', 'menu_background': '#D32F2F', 'txc_active': '#D32F2F', 'option_active': '#FFFFFF'}

mode = hc.nav_bar(menu_definition=menu_data, override_theme=over_theme, sticky_nav=True, sticky_mode='pinned')

# --- PAGE CONTENT ---
if mode == 'Home':
    st.markdown("<h1 style='text-align: center; color: #D32F2F;'>CASTRIES COMPREHENSIVE</h1>", unsafe_allow_html=True)
    st.markdown("<h3 style='text-align: center; color: #555555;'>Secondary School Portal</h3>", unsafe_allow_html=True)
    st.divider()

# --- STUDENT CHECK-IN INTERFACE ---
elif mode == "Student Check-in":
    st.markdown("<h2 style='text-align: center;'>--- STUDENT CHECK-IN ---</h2>", unsafe_allow_html=True)
    
    # STATUS BOX: Alerts students if they are LATE
    is_late = school_time.hour > 8 or (school_time.hour == 8 and school_time.minute > 15)
    if is_late:
        st.error(f"Current Status: LATE ({school_time.strftime('%I:%M %p')})") #
    else:
        st.success(f"Current Status: ON TIME ({school_time.strftime('%I:%M %p')})")

    val = st.text_input("Scan ID or Enter Name:", placeholder="Click here and scan card...").strip().lower() #

    if val:
        matched_key = get_close_matches(val, students.keys(), n=1, cutoff=0.6)
        if matched_key:
            display_name = matched_key[0].title()
            homeroom = students[matched_key[0]]
            
            if is_late:
                st.warning(f"⚠️ {display_name} recorded as LATE.")
                with open("detention.txt", "a") as f:
                    f.write(f"{display_name},{homeroom},{school_time.strftime('%I:%M %p')},{today_str}\n")
            else:
                st.balloons()
                st.success(f"✅ Welcome {display_name}! You are on time.")

# --- ADMINISTRATION PANEL ---
elif mode == "Teacher Attendance":
    st.markdown("<h2 style='color: #D32F2F;'>ADMINISTRATION PANEL</h2>", unsafe_allow_html=True) #
    pw = st.text_input("Enter Admin Password", type="password")
    
    if pw == "ccss2026":
        # METRIC CARDS
        view_date = st.selectbox("Select Date to View:", sorted(history_df['Date'].unique(), reverse=True) if not history_df.empty else [today_str])
        date_lates = history_df[history_df['Date'] == view_date]
        
        col1, col2, col3 = st.columns(3)
        col1.metric("Students Logged Late", len(date_lates)) #
        col2.metric("Total System Strikes", len(history_df)) #
        col3.metric("Portal Status", "ONLINE", delta="Stable") #
        style_metric_cards(background_color="#FFFFFF", border_left_color="#D32F2F")

        st.divider()
        st.subheader(f"Records for {view_date}")
        st.dataframe(date_lates, use_container_width=True) #
    elif pw != "":
        st.error("Incorrect Password.")
